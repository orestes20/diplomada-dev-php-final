...
<style>
    .format{
        background-color: #203767; border-top-left-radius: 20px;
        border-top-right-radius: 20px;
        border-bottom-right-radius: 20px;
        border-bottom-left-radius: 20px; color:white
    }
        ...
        <x-table :tableHead="$tableHead" :tableFilters="$tableFilters" :total="$total">
            @foreach ($aspirante as $key => $value)
                <tr>
                    ...
                    <td align="center"><input type="text" name="nota[]" id="nota{{$key}}" onkeyup="return notas({{$key}})" onkeypress="return SoloNumeros(event)" maxlength="2" size="3" class="form control"></td>                   
                </tr>
            @endforeach 
            <button class="format btn-rounted ">Guardar calificaciones</button>
        </x-table>
